...
model/effort flags, and JSON output. Commands in this playbook are
...
repo file and run one allowed command, then return `capability_ok: true`
only if both succeeded. Use a separate smoke prompt only when qualifying a
...
The orchestrator computes acceptance. All review forms require a successful
process/call, a valid JSON envelope, a schema-matching payload, the exact
dispatched pins, and `capability_ok` literally true. Missing evidence or
invalid output is never acceptance.

Extract a prose-wrapped payload when exactly one candidate JSON object
matches its schema. Zero or multiple matches: re-prompt once for
re-emission only, supplying the original output without an expected
verdict. Do not rerun a completed review for formatting. If still invalid,
record the outcome as contested and route it to the owner.
...
`clean` requires an empty findings list; `findings` requires a non-empty
list. Apply Verdict handling. Record a valid clean outcome with provenance
and stop the review pass; that does not close unfinished implementation.
...
3. Apply Verdict handling and the per-finding schema:
   ...
   {"verdict":"accepted|reopened|invalid","guard_confirmed":true,
    "capability_ok":true,"reviewed_sha":"<head-sha>",
    "base_sha":"<base-sha>","comments":["file:line — …"]}
   ```
   Acceptance additionally requires `guard_confirmed` literally true.
